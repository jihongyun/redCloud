...
public class SysMenuController extends AbstractController {
	 ...
	 */
	@RequestMapping("/nav")
	public R<List<NavMenuEntity>> nav(){
		List<NavMenuEntity> navList = sysMenuService.getNavMenuList(getUserId());
	...
	public R delete(long menuId){
		if(menuId <= 31){
			return R.error(501,"系统菜单，不能删除");
		}

		//判断是否有子菜单或按钮
		List<SysMenuEntity> menuList = sysMenuService.queryListParentId(menuId);
		if(menuList.size() > 0){
			return R.error(502,"请先删除子菜单或按钮");
		}
